...
export default class SpellLabyrinthRewardTreasureT2 extends ServerCard {
	...
	constructor(game: ServerGame) {
		...
		const chooseRewards = () => {
			this.cardsChosen = shuffle(CardLibrary.cards.filter((card) => card.features.includes(CardFeature.LABYRINTH_ITEM_T2))).slice(
				0,
				SpellLabyrinthRewardTreasureT2.REWARDS_OFFERED
			)
		}
