...
export default class SpellLabyrinthRewardLegendaryCard extends ServerCard {
	...
	constructor(game: ServerGame) {
		...
		const chooseRewards = () => {
			this.cardsChosen = shuffle(
				CardLibrary.cards.filter((card) => card.color === CardColor.GOLDEN).filter((card) => card.isCollectible)
			).slice(0, SpellLabyrinthRewardLegendaryCard.REWARDS_OFFERED)
		}
