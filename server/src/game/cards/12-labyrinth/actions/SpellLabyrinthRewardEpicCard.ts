...
export default class SpellLabyrinthRewardEpicCard extends ServerCard {
	...
	constructor(game: ServerGame) {
		...
		const chooseRewards = () => {
			this.cardsChosen = shuffle(
				CardLibrary.cards.filter((card) => card.color === CardColor.SILVER).filter((card) => card.isCollectible)
			).slice(0, SpellLabyrinthRewardEpicCard.REWARDS_OFFERED)
		}

		this.createDeployTargets(TargetType.CARD_IN_LIBRARY)
			.require(({ targetCard }) => this.cardsChosen.includes(targetCard))
			.perform(({ targetCard }) => {
				this.game.progression.labyrinth.addCardToDeck(targetCard.class, 2)
			})
	}
}
