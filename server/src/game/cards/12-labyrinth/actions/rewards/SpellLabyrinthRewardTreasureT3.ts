...
export default class SpellLabyrinthRewardTreasureT3 extends ServerCard {
	...
	constructor(game: ServerGame) {
		...
		const chooseRewards = () => {
			this.cardsChosen = shuffle(CardLibrary.cards.filter((card) => card.features.includes(CardFeature.LABYRINTH_ITEM_T3))).slice(
				0,
				SpellLabyrinthRewardTreasureT3.REWARDS_OFFERED
			)
		}
