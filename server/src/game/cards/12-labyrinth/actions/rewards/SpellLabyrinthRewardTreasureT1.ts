...
export default class SpellLabyrinthRewardTreasureT1 extends ServerCard {
	...
	constructor(game: ServerGame) {
		...
		const chooseRewards = () => {
			this.cardsChosen = shuffle(CardLibrary.cards.filter((card) => card.features.includes(CardFeature.LABYRINTH_ITEM_T1))).slice(
				0,
				SpellLabyrinthRewardTreasureT1.REWARDS_OFFERED
			)
		}
