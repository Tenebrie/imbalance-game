...
class CardImageGenerator {
	public generatePlaceholderImages(): void {
		const productionDir = path.join(ROOT_DIR, '../../client/assets/cards')
		const placeholderDir = path.join(ROOT_DIR, 'generated/assets/cards')
	...
	private async generatePlaceholderImage(cardClass: string, magicString: string, reason: 'placeholder' | 'workshop'): Promise<Buffer> {
		...
		const filepath = path.join(ROOT_DIR, 'src/assets/bg-clean.png')
	...
	public async generateStaticPlaceholderImage(cardClass: string, magicString: string): Promise<void> {
		...
		const dir = path.join(ROOT_DIR, 'generated/assets/cards')
