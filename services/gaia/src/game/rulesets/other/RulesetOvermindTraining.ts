...
type Response = {
	victoriousAgent: string | null
}

export default class RulesetOvermindTraining extends ServerRuleset {
	...
	public runSimulation(props: ExtraProps): Promise<Response> {
		return new Promise<Response>((resolve) => {
			...
			players[0].setOvermindId(props.firstAgentId)
			players[1].setOvermindId(props.secondAgentId)

			this.createCallback(GameEventType.GAME_FINISHED).perform(({ victoriousPlayer }) => {
				resolve({
					victoriousAgent: victoriousPlayer === null ? null : victoriousPlayer.index === 0 ? props.firstAgentId : props.secondAgentId,
				})
			})
			this.game.start()
		})
