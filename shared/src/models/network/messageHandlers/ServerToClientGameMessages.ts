...
import GroupAnimationThreadNext from '../groupWrappers/GroupAnimationThreadNext'
import GroupGenericLoop from '../groupWrappers/GroupGenericLoop'
...
export type GroupMessageTypeMapping = {
	[GroupMessageType.CARD_BUFF]: null
	[GroupMessageType.THREAD_NEXT]: GroupAnimationThreadNext
	[GroupMessageType.GENERIC_LOOP]: GroupGenericLoop
}

export type GroupMessageHandlers = ConsumerSideTyper<GroupMessageTypeMapping>
...
type ConsumerSideTyper<T> = { [K in keyof T]: (data: T[K], systemData: QueuedMessageSystemData) => void }
